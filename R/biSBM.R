...
#
# OUTPUTS:
# obj         - An object of class biSBM obj containing the final partition ( N x 1 vector) and
#               two attributes: score (final likelihood score), nodeTypes (vector of node types).
...
biSBM <- function(data, nodeType, ka, kb, deg.corr=1, iter=10){
  ...
  partition <-integer(length(nodeType));
  score <- double(1);
  output <- .C("rFunction", as.numeric(data), nrow(data), nodeType, length(nodeType), as.integer(ka),
     as.integer(kb), as.integer(deg.corr), as.integer(iter), res=partition,  MLScore = score);

  obj <- new_biSBMObj(partition = output$res+1, score = output$MLScore, nodeType)
  obj
}
